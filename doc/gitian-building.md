...
*Setup instructions for a Gitian build of Pythonbit using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Pythonbit
...
- [Building Pythonbit](#building-pythonbit)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-pythonbit-host/README.md).
...
Clone the git repositories for pythonbit and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/pythonbit/pythonbit
```
...
in the pythonbit repository under 'Fetch and build inputs' to install sources which require
...
To build Pythonbit (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the pythonbit repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/pythonbit/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/pythonbit/pythonbit
...
For example:
```bash
URL=https://github.com/laanwj/pythonbit.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit pythonbit=${COMMIT} --url pythonbit=${URL} ../pythonbit/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit pythonbit=${COMMIT} --url pythonbit=${URL} ../pythonbit/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit pythonbit=${COMMIT} --url pythonbit=${URL} ../pythonbit/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the pythonbit git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../pythonbit/contrib/gitian-descriptors/*|sort|uniq )
...
PBTPATH=/some/root/path/pythonbit.git
SIGPATH=/some/root/path/pythonbit-detached-sigs.git

./bin/gbuild --url pythonbit=${PBTPATH},signature=${SIGPATH} ../pythonbit/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/pythonbit-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/pythonbit-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/pythonbit-osx-build.assert
```
...
[pythonbit/gitian.sigs](https://github.com/pythonbit/gitian.sigs/) repository, or if that's not possible create a pull
