Pythonbit Core version 0.11.0 is now available from:

  <https://pythonbit.org/bin/pythonbit-core-0.11.0/>
...
backwards-compatible with pre-0.10 versions of Pythonbit Core or other software:
...
For example, add the following to `pythonbit.conf`:
...
related to the blockchain in the pythonbit system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Pythonbit Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Pythonbit Core will 
...
For an example script see [pythonbit-submittx](https://github.com/laanwj/pythonbit-submittx).
...
- #5334 `cf87536` libpythonbitconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new pythonbit-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only PythonbitAmountField
- #5753 `0f44672` Add pythonbit logo to about screen
...
- #5809 `46bfbe7` Add pythonbit-cli man page
...
- #5849 `41259ca` contrib/init/pythonbitd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Pythonbit-Qt.app to Pythonbit-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for pythonbit-tx)
