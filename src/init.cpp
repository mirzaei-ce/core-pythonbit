...
void Shutdown()
{
    ...
    RenameThread("pythonbit-shutoff");
...
#endif
    GeneratePythonbits(false, 0, Params());
    StopNode();
    StopTorControl();
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), PYTHONBIT_CONF_FILENAME));
    if (mode == HMM_PYTHONBITD)
    {
#ifndef WIN32
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), PYTHONBIT_PID_FILENAME));
#endif
...
std::string LicenseInfo()
{
    ...
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Pythonbit Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
void ThreadImport(std::vector<boost::filesystem::path> vImportFiles)
{
    ...
    RenameThread("pythonbit-loadblk");
    // -reindex
    if (fReindex) {
        CImportingNow imp;
        int nFile = 0;
...
/** Sanity checks
 *  Ensure that Pythonbit is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Pythonbit version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
}

/** Initialize pythonbit.
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Pythonbit Core is shutting down."));
...
#endif
    // Make sure only a single Pythonbit process is using the data directory.
    ...
    try {
        ...
        if (!lock.try_lock())
            return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Pythonbit Core is probably already running."), strDataDir));
    } catch(const boost::interprocess::interprocess_exception& e) {
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Pythonbit Core is probably already running.") + " %s.", strDataDir, e.what()));
    }
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Pythonbit Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Pythonbit Core to complete") << "\n";
    ...
    GeneratePythonbits(GetBoolArg("-gen", DEFAULT_GENERATE), GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
