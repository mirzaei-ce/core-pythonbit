...
{
    std::cout <<
        "This program outputs Pythonbit addresses and private keys from a wallet.dat file" << std::endl
        << std::endl
        << "Usage and options: "
        << std::endl
...
std::string WalletUtilityDB::getAddress(CDataStream ssKey)
{
    ...
    std::string strAddr = CPythonbitAddress(id).ToString();
...
std::string WalletUtilityDB::getKey(CDataStream ssKey, CDataStream ssValue)
{
    ...
    if (key.Load(pkey, vchPubKey, true))
        strKey = CPythonbitSecret(key).ToString();
...
std::string WalletUtilityDB::getCryptedKey(CDataStream ssKey, CDataStream ssValue, std::string masterPass)
{
    ...
    std::string strKey = CPythonbitSecret(key).ToString();
    return strKey;
}
