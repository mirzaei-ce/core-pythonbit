    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++) {
            ...
            for (size_t j = 0; j < tx.vin.size(); j++) {
                ...
                if (GetSpentIndex(spentKey, spentInfo)) {
                    if (spentInfo.addressType == 1) {
                        delta.push_back(Pair("address", CPythonbitAddress(CKeyID(spentInfo.addressHash)).ToString()));
                    } else if (spentInfo.addressType == 2)  {
                        delta.push_back(Pair("address", CPythonbitAddress(CScriptID(spentInfo.addressHash)).ToString()));
                    } else {
                        continue;
                    }
        ...
        for (unsigned int k = 0; k < tx.vout.size(); k++) {
            ...
            if (out.scriptPubKey.IsPayToScriptHash()) {
                ...
                delta.push_back(Pair("address", CPythonbitAddress(CScriptID(uint160(hashBytes))).ToString()));

            } else if (out.scriptPubKey.IsPayToPublicKeyHash()) {
                vector<unsigned char> hashBytes(out.scriptPubKey.begin()+3, out.scriptPubKey.begin()+23);
                delta.push_back(Pair("address", CPythonbitAddress(CKeyID(uint160(hashBytes))).ToString()));
            } else {
                continue;
            }
...
UniValue gettxout(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of pythonbit addresses\n"
