...
###   This script attempts to download the signature file SHA256SUMS.asc from pythonbit.org
...
WORKINGDIR="/tmp/pythonbit"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://pythonbit.org/bin/"
VERSIONPREFIX="pythonbit-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'pythonbit-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Pythonbit Core binary release signing key installed?"
   fi
